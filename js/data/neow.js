...
/**
 * 테스트용 시작 보너스 — 최대 체력 · 공격 카드 피해 · 방어 카드 방어도 ·
 * 물약 효과 · 골드가 모두 2배가 되고 일반 유물 1개와 엘리트 유물 1개를 들고 시작한다.
 *
 * v1.3.16 까지는 시작 보너스 화면의 '아무 표시 없는 다섯 번째 자리' 로 눌렀지만,
 * v1.3.17 부터는 관리자 모드의 "자원 2배로 시작" 이 이 자리를 대신한다.
 * (시작 보너스 화면의 숨은 버튼은 없앴다 — 모르고 눌러 판을 버리는 일이 있었다)
 *
 * run.cheat 를 켜므로 이 판은 전적에 남지 않는다.
 */
