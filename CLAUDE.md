...
* **아이폰이 알려 주는 안전 여백을 믿지 말 것** (v1.5.4). `black-translucent` 로 설치된
  홈 화면 앱은 `env(safe-area-inset-*)` 을 실제보다 작게 준다 — 아이폰 15 Pro Max 에서
  **위 30 / 아래 22** 로 왔다 (실제는 59 / 34). 그대로 믿으면 맨 윗줄이 상태바·다이내믹
  아일랜드 밑에 깔리고 **그 자리의 탭은 iOS 가 가져간다** (버튼이 보여도 눌리지 않는다).
  그래서 `html.ios-app` (index.html 인라인 스크립트가 iOS+홈 화면 앱에만 붙인다) 에서
  `--safe-t` 바닥값을 `min(아래 × 3, 62px)` 로 잡는다 — 값이 작게 와도 62px 을 확보하고,
  제대로 오는 기기도 같은 62px 에 걸리며, 홈 버튼 기기(아래 0)는 영향이 없다.
  meta 의 `black-translucent` 를 `black` 으로 바꾸지 말 것 — 여백이 두 배가 된다.
  자주 누르는 것을 화면 꼭대기에 두지 말 것 — 메뉴 버튼이 맨 윗줄이 아니라
  **2행(유물 줄) 오른쪽 끝**에 있는 이유다 (v1.5.3).
  의심스러우면 관리자 모드 → 이 기기 정보의 `표시 모드` · `안전 여백` · `메뉴 버튼 위치` 를 볼 것
