...
# 헤드리스 전투 시뮬 (캐릭터 4종 × 전 카드 × 강화 전/후 = 2904회)
node tools/sim-cards.mjs   # 엔진 구조를 건드렸으면 반드시
...
* **아이폰 홈 화면 앱에서 화면 맨 위·맨 아래에 누르는 것을 두지 말 것** (v1.5.2).
  ...
  메뉴 버튼은 그래서 맨 윗줄이 아니라 **2행(유물 줄) 오른쪽 끝**에 있다 (v1.5.3) —
  자주 누르는 것을 화면 꼭대기에 두지 말 것
...
* 새 힘/상태이상은 `js/engine/powers.js` 에 정의 + `js/ui/powerart.js` 에 **고유한 상태 아이콘**을
  그리고 **`node tools/check-powerart.mjs` 를 돌릴 것** (v1.5.0). 118종이 전부 달라야 한다 —
...
* **백과사전의 긴 설명에는 수치를 다시 적지 말 것** (v1.5.1). `js/data/statehelp.js`(상태 118종) ·
  `js/data/cardhelp.js`(카드) 는 바로 위에 이미 있는 한 줄/카드 글 **밑에** 붙는 글이다.
  수치를 또 적으면 밸런스를 고칠 때 반드시 갈라진다 — 여기에는 **언제 붙고 언제 사라지는지 ·
  무엇에 적용되는지 · 흔한 오해**만 적는다. `**굵게**` 만 쓸 수 있고 `richHTML()` 을 거쳐 그려진다.
  용어집에서 상태와 이름이 같은 낱말은 `statehelp.js` 것을 자동으로 끌어다 쓰므로 두 번 적지 말 것
  (이름이 겹치는 자세 "분노" 처럼 뜻이 다른 것만 사전에 직접 적는다)
* **구체 동작은 하나씩 사이를 띄운다** (v1.5.1). `channel` · `evoke` · `evokeFront` · `evokeAll` ·
  `orbPassives` 는 `async` 이고 `pace()` 로 `ORB_GAP` 만큼 쉰다 — 이게 없으면 이중 시전의 번개 두 번이
  한 번처럼 보인다. **반드시 `await` 할 것.** 기다릴 수 없는 곳(피해 처리 도중 · 전투 시작 유물)은
  사이를 띄우지 않는 `channelOne()` 을 쓴다. UI(`main.js` 의 `orbFire`)에서 또 시차를 주지 말 것 — 겹으로 밀린다
* 카드 글에 상태(힘/상태이상) 이름이 나오면 `js/data/keywords.js` 에 뜻풀이를 더하고
  **`node tools/check-keywords.mjs` 를 돌릴 것** (v1.4.2). 카드에는 "조준을 3 부여합니다" 라고만
  적히므로 그 말뜻은 사전에만 있다 — 빠뜨리면 카드 상세의 "용어" 칸이 통째로 안 뜬다.
  백과사전 → 용어 → **상태** 목록은 `POWERS` 를 그대로 받아 적으니 따로 손댈 곳이 없다
