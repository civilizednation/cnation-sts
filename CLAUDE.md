...
* **아이폰 여백에 `env(safe-area-inset-*)` 을 쓰지 말 것** (v1.5.5). 홈 화면 앱에서 이 값이
  실제보다 작게 오거나 **0 으로 온다** — 그래서 v1.5.2~1.5.4 의 보정이 세 번 다 0 이 되어
  화면이 한 픽셀도 안 움직였다 (env 에 곱하거나 비례시키면 0 에서 무너진다).
  지금은 `index.html` 의 인라인 스크립트가 **화면 비율**(`세로/가로 ≥ 2.1` 이면 노치·아일랜드
  아이폰)로 판단해 `--safe-t: max(env(...), 62px)` · `--safe-b: max(env(...), 30px)` 을
  **인라인 스타일로 직접** 심는다. 스타일시트에서 다시 정하지 말 것 — 그 값과 싸우게 된다.
  맨 윗줄이 상태바·다이내믹 아일랜드에 깔리면 **그 자리의 탭은 iOS 가 가져가서** 버튼이
  보여도 눌리지 않는다. 자주 누르는 것을 화면 꼭대기에 두지 말 것 — 메뉴 버튼이 맨 윗줄이
  아니라 **2행(유물 줄) 오른쪽 끝**에 있는 이유다 (v1.5.3).
  meta 의 `black-translucent` 를 `black` 으로 바꾸지 말 것 — 여백이 두 배가 된다.
  확인은 관리자 모드 → 이 기기 정보의 `표시 모드` · `기기 화면` · `안전 여백` · `메뉴 버튼 위치`
