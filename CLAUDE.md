...
js/safearea.js             아이폰 상태바 여백 (env 를 믿지 않고 화면 비율로 판단 · 손조정 저장)
...
* **화면 여백은 `js/safearea.js` 가 정한다** (v1.5.6). 두 가지가 겹친 문제였다.
  ① iOS 홈 화면 앱은 `env(safe-area-inset-*)` 을 실제보다 작게 주거나 **0 으로 준다** —
  env 에 곱하거나 비례시키면 0 에서 무너진다 (v1.5.2~1.5.4 가 세 번 다 그랬다).
  ② 그 계산을 `index.html` 인라인 스크립트에 두면 **홈 화면 앱이 그 문서를 캐시에 묶어 둬서**
  js 는 새것인데 html 은 옛것이 된다 (타이틀 버전만 올라가고 배치는 그대로 — v1.5.5 가 그랬다).
  그래서 판단 기준은 **화면 비율**(`세로/가로 ≥ 2.1` 이면 노치·아일랜드 아이폰 → 위 62px)이고,
  값을 심는 자리는 **js 모듈**이다 (`js/main.js` 가 맨 먼저 `applySafeArea()` 를 부른다).
  `index.html` 의 사본은 깜빡임을 없애려는 덤이니 그것만 고치지 말 것.
  **아래쪽은 건드리지 않는다** — 늘리면 손패가 좁아지고, 실제 기기에서 충분한 것을 확인했다.
  맨 윗줄이 상태바에 깔리면 **그 자리의 탭은 iOS 가 가져가서** 버튼이 보여도 눌리지 않으므로,
  자주 누르는 것을 화면 꼭대기에 두지 말 것 (메뉴 버튼이 2행 오른쪽 끝에 있는 이유 · v1.5.3).
  meta 의 `black-translucent` 를 `black` 으로 바꾸지 말 것 — 여백이 두 배가 된다.
  기기마다 다를 때는 관리자 모드 → **화면 여백 조정** 으로 손수 맞출 수 있다 (그 기기에만 저장).
  지금 기본값은 **위 82px** 이다 (v1.5.7 · 실제 아이폰에서 눈으로 맞춘 값 · 상태바 59 + 여유 23).
  자동값을 또 바꾸면 `safearea.js` 의 `VER` 을 올릴 것 — 안 올리면 옛 손조정이 겹쳐 두 배가 된다.
...
* **방에 들어가기 전에 저장하고, 전투 중에는 저장하지 말 것** (v1.5.8). 저장 형식에는
  전투가 담기지 않는다 — 전투 중에 저장하면 남는 것은 "그 방을 지나왔다" 는 사실뿐이라,
  앱을 강제로 닫았을 때 그 전투가 통째로 건너뛰어진다 (보상도 없이).
  `enterRoom()` 은 `run.enterNode()` **앞**에서 `saveRun` 하고, `visibilitychange` 는
  `!G.battle` 일 때만 저장한다. 씨앗도 그 시점 그대로라 다시 열면 같은 전투를 처음부터 치른다.
  전투 상태를 통째로 저장하는 쪽으로 바꾸고 싶다면 `Battle` 을 직렬화해야 한다 — 지금은 안 한다
